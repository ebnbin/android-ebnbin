...
object IntentHelper {
    ...
    fun restartApp(): Boolean {
        val intent = EBApplication.instance.packageManager.getLaunchIntentForPackage(
            EBApplication.instance.applicationId) ?: return false
        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK)
        return startActivity(EBApplication.instance, intent)
    }
    ...
    fun startMarket(context: Context): Boolean {
        val intent = Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=${EBApplication.instance.applicationId}"))
        val result = startActivity(context, intent)
        if (!result) {
    ...
    fun startSettingsFromFragment(fragment: Fragment, action: String, requestCode: Int): Boolean {
        val intent = Intent(action, Uri.parse("package:${EBApplication.instance.applicationId}"))
        return startActivityFromFragment(fragment, intent, requestCode)
    }
}
